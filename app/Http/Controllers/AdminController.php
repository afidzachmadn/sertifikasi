...
//---------------------------------------------------------------------------------------------//
  ...
  public function proses_batal_verifikasi_sni(Request $request){
   ...
   $status_sni = $request->get('texted');
   $userDb = DB::table('users')->where('id', $id)
                               ->update(['status_SNI' => $status_sni,'Verifikator_SNI_name' => $name,'surat_pengesahan_SNI'=>'default-pengesahan-sni.pdf', 'Verifikator_ID_SNI' => '']);
...
//---------------------------------------------------------------------------------------------//
  public function pegawai_inspeksi_sni(Request $request){
  // $email = $request->session()->get('email');
   ...
   $status = '';
      ...
      if($request->session()->get('login')) {
          return view('admin.pilih-pegawai-inspeksi-sni', array('userList'=>$userList,'pegawaiList' => $pegawaiList, 'status'=>$status));
      } else {
          return view('auth.login-admin');
      }
  }

  public function pegawai_inspeksi_sni_proses(Request $request){
  ...
  $pilihan_perusahaan=$request->get('pilihan-perusahaan');
  $nama_petugas_inspeksi_sni= DB::table('pegawai_lapangan')->where('nip',$nip)->get();
  $nama_petugas_inspeksi_sni_next = $nama_petugas_inspeksi_sni[0]->name;

  
  
  $userDb = DB::table('users')->where('company_name', $pilihan_perusahaan)
                               ->update(['petugas_inspeksi_SNI_name' => $nama_petugas_inspeksi_sni_next,'petugas_inspeksi_nip_SNI' => $nip,'petugas_inspeksi_SNI_ID' => $id]);

  $inspeksiSNIHitung = DB::table('users')->where('petugas_inspeksi_SNI_ID', $id)->count();

  $petugasDb = DB::table('pegawai_lapangan')->where('nip', $nip)
                               ->update(['jumlah_total_pendaftar_SNI_yang_diverifikasi' => $inspeksiSNIHitung]);

   return redirect()->action('AdminController@pegawai_inspeksi_sni');
  }
...
//--------------------------------------------------------------------------------------------------//
public function riwayat_pegawai_SNI(Request $request){
  
}

public function riwayat_pegawai_SNI_process(Request $request){
   $email = $request->session()->get('email');
   $id = $request->get('no');
   //dd($id);

   $userList = DB::table('users')->where([['petugas_inspeksi_SNI_ID','=',$id],])->get();

  // $surat_inspeksi = DB::table('users')->where([['surat_inspeksi_SNI','=',null],['surat_inspeksi_SNI','!=',null],])->get();
   ...
   //dd($surat_inspeksi);
      ...
      if($request->session()->get('login')) {
          return view('admin.riwayat-pegawai-inspeksi-sni', array('userList' => $userList));
      } else {
          return view('auth.login-admin');
      }
...
public function input_pegawai_SNI_process(Request $request){
 ...
 // dd($jabatan);
  ...
  $petugasDb = DB::table('pegawai_lapangan')->where('nip', $nip)
                               ->insert(['jumlah_total_pendaftar_SNI_yang_diverifikasi' => $total_pendaftar_sni_yang_diverifikasi,
                               'jumlah_total_pendaftar_ISO_yang_diverifikasi' =>
                               $total_pendaftar_iso_yang_diverifikasi,
                               'name' => $nama_pegawai,
                               'nip' => $nip,
                               'jabatan' => $jabatan,
